...
class FTS3OAuth2ResourceProvider(ResourceProvider):
    ...
    def obtain_refresh_token_from_auth(self, authorization):
        ...
        (fts_token, refresh_token) = oidc_manager.generate_refresh_token(
            issuer=authorization.issuer,
            token=authorization.token,
            audience=audience,
            scope=authorization.scope,
        )
        return fts_token, refresh_token

    def validate_fts_token(self, fts_token, authorization):
        """
        ...
        try:
            if not oidc_manager.token_issuer_supported(fts_token):
        ...
        try:
            if validation_method == "offline":
                valid, credential = self._validate_token_offline(fts_token)
            else:
                valid, credential = self._validate_token_online(fts_token)
            if not valid:
                return
        except Exception as ex:
            log.warning(
    ...
    def _validate_token_offline(self, fts_token):
        """
        ...
        unverified_header = jwt.get_unverified_header(fts_token)
    ...
    def _validate_token_online(self, fts_token):
        """
        ...
        """
        unverified_payload = jwt.decode(fts_token, options=jwt_options_unverified())
        issuer = unverified_payload["iss"]
        log.debug("issuer={}".format(issuer))
        credential = oidc_manager.introspect(issuer, fts_token)
