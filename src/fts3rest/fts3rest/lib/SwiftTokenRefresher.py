...
from fts3rest.model import Credential, Host, Job
...
class SwiftTokenRefresher(Thread):
    """
    Daemon thread that refreshes all Swift OS tokens related to unfinished transfers in the DB at every interval.
    ...
    def run(self):
        ...
        while True:
            # Check that no other fts-swift-token-refresh-daemon is running
            refresher_threads = (
            ...
            if all(self._thread_is_inactive(thread) for thread in refresher_threads):
                ...
                while True:
                    ...
                    try:
                        ...
                        Session.commit()
                        log.debug(
                            "fts-swift-token-refresh-daemon heartbeat {}".format(h2.beat)
                        )
                    except SQLAlchemyError as ex:
                        log.warning(
                            "Failed to update the fts-swift-token-refresh-daemon heartbeat: %s"
                            % str(ex)
                        )
                        Session.rollback()
                        raise
                    ...
                    jobs = (
                        Session.query(Job)
                        .filter(((Job.job_state == "Submitted") | (Job.job_state == "Active")) &
                                ((Job.source_se.like("swift%")) | (Job.dest_se.like("swift%"))))
                        .all()
                    )
