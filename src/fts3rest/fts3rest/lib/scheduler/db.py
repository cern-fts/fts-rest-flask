...
class Database:
    ...
    def get_submitted(self, src, dst, vo):
        ...
        """
        queue = (
            self.session.query(func.count(File.source_se))
    ...
    def get_success_rate(self, src, dst):
        ...
        sum_ = 0
        arr = (
            self.session.query(OptimizerEvolution.success)
            .filter(OptimizerEvolution.source_se == src)
            .filter(OptimizerEvolution.dest_se == dst)
            .filter(
                OptimizerEvolution.datetime >= (datetime.utcnow() - timedelta(hours=1))
            )
        )
        size = 0
        for x in arr:
            sum_ += x[0]
            size += 1
        return 100 if sum_ == 0 else sum_ / size
    ...
    def get_throughput(self, src, dst):
        ...
        for tp, active in (
            self.session.query(OptimizerEvolution.throughput, OptimizerEvolution.active)
            .filter(OptimizerEvolution.source_se == src)
            .filter(OptimizerEvolution.dest_se == dst)
            .filter(
                OptimizerEvolution.datetime >= (datetime.utcnow() - timedelta(hours=1))
    ...
    def get_per_file_throughput(self, src, dst):
        ...
        for per_file_throughput in (
            self.session.query(OptimizerEvolution.throughput)
            .filter(OptimizerEvolution.source_se == src)
            .filter(OptimizerEvolution.dest_se == dst)
            .filter(
                OptimizerEvolution.datetime >= (datetime.utcnow() - timedelta(hours=1))
    ...
    def get_pending_data(self, src, dst, vo, user_activity):
        ...
        if share is None:
            for data in (
                self.session.query(File.user_filesize)
                .filter(File.source_se == src)
                .filter(File.dest_se == dst)
                .filter(File.vo_name == vo)
                .filter(File.file_state == "SUBMITTED")
