...
def get_column_type_description(sql_type):
    ...
    """
    if (
        isinstance(sql_type, types.String)
        or isinstance(sql_type, Json)
        or isinstance(sql_type, TernaryFlag)
        or isinstance(sql_type, types.CLOB)
    ):
        return {"type": "string"}
...
def get_relation_type_description(prop, model_list):
    ...
    if prop.uselist:
        return {"type": "array", "items": {"$ref": ref_type}}
    else:
...
def get_mapper():
    ...
    """
    mock_config = {"pylons.paths": {"controllers": CONTROLLERS_PATH}, "debug": False}
...
def get_routes(mapper):
    ...
    """
    return filter(
        lambda r: not r.routelist[0].startswith("/error/") and r.routelist[0] != "/",
...
def generate_resources(routes, controllers):
    ...
    for cname in filter(
        lambda cname: cname,
        map(lambda route: route.defaults.get("controller", None), routes),
    ):
        if cname not in resources:
            if str(cname) in controllers:
                ...
                doc = None
            resources[cname] = {"id": cname, "description": doc}
...
def get_function_responses(function):
    ...
    for code, description in getattr(function, "doc_responses", []):
        responses.append({"code": code, "message": description})
    return responses
    ...
    for route in routes:
        ...
        if cname and str(cname) in controllers:
            ...
            for action in actions:
                ...
                api = {"path": path, "operations": operations}
