...
# For the migration to Python 3, I replaced types.StringType to str
# as explained here https://stackoverflow.com/questions/4232111/stringtype-and-nonetype-in-python3-x
...
def check_string_length(cls, key, inst):
    ...
    if isinstance(prop, ColumnProperty) and len(prop.columns) == 1:
        col = prop.columns[0]
        if (
            isinstance(col.type, String) or isinstance(col.type, Json)
        ) and col.type.length:
            ...
            def _set(instance, value, oldvalue, initiator):
                ...
                if value and len(value) > max_length:
                    raise ValueError(
                        "Length %d exceeds allowed %d" % (len(value), max_length)
                    )
