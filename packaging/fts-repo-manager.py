...
def sh(cmd):
    ...
    process = subprocess.Popen(
        cmd, stdout=subprocess.PIPE, shell=True, stderr=subprocess.STDOUT
    )
    ...
    if retcode:
        raise Exception(
            "Command {0} exited with code {1}. Output: {2}".format(cmd, retcode, output)
        )
    ...
    if choice:
        ...
        for item in choice:
            if item not in available:
                parser.error(
                    "unrecognized {0}: '{1}'. Available choices: {2}".format(
                        text, item, available
                    )
                )
...
def add_dependency(parser, when_present, dependency):
    if hasattr(parser, when_present):
        if not hasattr(parser, dependency):
            parser.error(
                "argument --{0} is required when --{1} is present".format(
                    dependency, when_present
                )
            )
...
class Repository(object):
    ...
    def store(self, ref, packages, arch_dir):
        ...
        if len(archs) > 1:
            raise ValueError(
                "Cannot mix packages of different architectures in the same invocation: {0}".format(
                    list(archs)
                )
            )
        elif len(archs) == 0:
            raise ValueError(
                "Only noarch packages: Provide a arch directory to deploy the packages."
            )
...
def declare_incompatible_options(parser, option, group):
    ...
    for item in group:
        if item in sys.argv:
            parser.error(
                "argument {0} is incompatible with argument {1}".format(option, item)
            )
...
def parseargs():
    ...
    parser.add_argument(
        "--base", type=str, required=True, help="The base directory for your project."
    )
    parser.add_argument(
        "--action",
        type=str,
        required=True,
        help="The action to perform. Choices: ['add', 'cleanup']",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="If set, don't actually change any files, just show what would happen if ran.",
    )
    parser.add_argument(
        "--no-create-repo",
        action="store_true",
        help="If set, don't run createrepo at the end.",
    )
    ...
    group.add_argument(
        "--ref",
        type=str,
        help="The branch or tag that is being built. Tag names must match 'x.y' or 'x.y.z' (may be prepended by 'v')",
    )
    group.add_argument(
        "--packages", type=str, nargs="+", help="The list of packages to add"
    )
    group.add_argument(
        "--arch-dir",
        type=str,
        help="In case of noarch packages specify in which arch directory the packages are deployed.",
    )
    ...
    group.add_argument(
        "--keep-last-days",
        type=int,
        help="How many days worth of RPMs to keep. (only affects branches)",
    )
